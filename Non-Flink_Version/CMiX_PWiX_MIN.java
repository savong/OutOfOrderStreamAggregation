...
public class CMiX_PWiX_MIN {
    ...
    public static void main(String[] args) throws UnknownHostException, IOException, ClassNotFoundException, InterruptedException{
        ...
        //Becase we compute minimum value, so the default values in both cmix and pwix must be set to DOUBLE.MAX_VALUE
        for(int i=0;i<n;i++)
        {
            cmix[i] = Double.MAX_VALUE;
        }
        for(int i=0;i<p;i++)
        {
            pwix[i] = Double.MAX_VALUE;
        }
            ...
            while((line = in.readLine()) != null) {
                ...
                if((current_time-Long.parseLong(cells[0]))/10000<=max_lateness)
                {
                    ...
                    if(duration < (round+1)*f1)
                    {
                        ...
                        {
                            pvalue_perslide.put(index_falling, Math.min(pvalue_perslide.get(index_falling), Double.valueOf(cells[1])));
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        for (Map.Entry<Integer, Double> entry : pvalue_perslide.entrySet())
                        {
                            ...
                            if(entry.getKey()==round)
                            {
                                ...
                                {
                                    cmix[(int)(Math.floor(round/x)*x)] = Math.min(cmix[(int)(Math.floor(round/x)*x)], entry.getValue());
                                }
                                //Aggregate into the 0 th partition if current slide is not pointing to the 0th index in CMiX
                                //Line 10 in Algorithm 1
                                if(entry.getKey()!=0)
                                {
                                    cmix[0] = Math.min(cmix[0], entry.getValue());
                                }
                            }
                            else //Line 11-21 in Algorithm 1
                            {
                                ...
                                cmix[entry.getKey()] = Math.min(cmix[entry.getKey()], entry.getValue());
                                ...
                                {
                                    cmix[(int)(Math.floor(entry.getKey()/x)*x)] = Math.min(cmix[(int)(Math.floor(entry.getKey()/x)*x)], entry.getValue());
                                }
                            }
                            ...
                            if(entry.getKey() > round)
                            {
                                cmix[next_k*x] = Math.min(cmix[next_k*x], entry.getValue());
                            }
                            else
                            {
                                cmix[0] = Math.min(cmix[0], entry.getValue());
                        ...
                        if(round%x == x-1 || round == n-1)
                        {
                            ...
                            {
                                cmix[next_k*x] = Math.min(cmix[next_k*x], cmix[j*x]);
                            }
                        }
        ...
        /*else
            {
                ...
                //We have not implemented this case yet, but will do so soon.
            }*/
    }
    ...
    public static double Compute_Result(double[] cmix, double Result, int k, int next_k, int m, int n, int x)
    {
        ...
        if(k < Math.ceil(n/x)-1)
        {
            ...
            {
                Result = Math.min(cmix[0], cmix[next_k*x]);
            }
            else//Otherwise (Lines 5-6 in Algorithm 2)
            {
                Result = Math.min(cmix[0], Math.min(cmix[next_m], cmix[next_k*x]));
            }
        }
        else //If the current block is the ⌈n/x⌉ − 1 th block (Line 8 in Algorithm 2):
        {
            ...
            {
                Result = Math.min(cmix[0], cmix[next_m]);
            }
    ...
    public static double[] Aggkplus1Block(double[] cmix, int k, int next_k, int m, int n, int x)
    {
        ...
        {
            cmix[next_k*x+i] = Math.min(cmix[next_k*x+i], cmix[next_k*x+i+1]);
        }
        return cmix;
    }
    ...
    public static double[] UpdatePCResult(double[] pwix, Map<Integer, Double> pvalue_perslide, int p, int m, double R, long start_time, int slide)
    {
        ...
        double la_pre = Double.MAX_VALUE;
    ...
    public static Pair_value UPResult(double[] pwix, Map<Integer, Double> pvalue_perslide_in_pwix, ArrayList<Integer> sortedKeys, double la_pre, int lst, boolean have_right, long start_time, int slide)
    {
        ...
        for(int i=l; i<lst; i++)
        {
            ...
            {
                la_pre = Math.min(la_pre, pvalue_perslide_in_pwix.get(i));
            }
            pwix[i] = Math.min(pwix[i], la_pre);
