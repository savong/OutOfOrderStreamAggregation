...
/*
STANDARD DEVIATION (STD) = sqrt( (1/N) * sum^N_i (x_i - x_mean)^2 ).
Do some simplification:
    sum^N_i (x_i - x_mean)^2 = sum^N_i (x_i^2 - 2*x_i*x_mean + x_mean^2)
    =  sum^N_i (x_i^2) - 2*x_mean*sum^N_i (x_i) + N*x_mean^2)


As long as we can incrementally compute sum of square (sum^N_i (x_i^2)), sum (sum^N_i (x_i)), and count (N),
we can also incrementally compute STD. Implementation is based on this logic.
*/
public class CMiX_PWiX_STD_using_Flink {
    public static void main(String[] args) throws Exception {
        ...
        if(f2==0)//If window size is dividiable by slide size
        {
            ...
            .reduce(new ReduceFunction<Tuple2<String,Double>>(){
                count_aggregation[] cmix = new count_aggregation[n];
                count_aggregation[] pwix = new count_aggregation[p];
                    ...
                    if(current_time-Long.parseLong(v2.f0)<=max_lateness)
                    {
                        ...
                        if(duration < (round+1)*f1)
                        {
                            if(pvalue_perslide.containsKey(index_falling))
                            {
                                ...
                                co_ag_pair.count = co_ag_pair.count + 1;
                                co_ag_pair.aggregation = co_ag_pair.aggregation + v2.f1;
                                co_ag_pair.aggregation_square = co_ag_pair.aggregation_square + v2.f1*v2.f1;
                            ...
                            else
                            {
                                ...
                                co_ag_pair.count = 1;
                                co_ag_pair.aggregation = v2.f1;
                                co_ag_pair.aggregation_square = v2.f1*v2.f1;
                                pvalue_perslide.put(index_falling, co_ag_pair);
                            }
                        ...
                        else
                        {
                            ...
                            for (Map.Entry<Integer, count_aggregation> entry : pvalue_perslide.entrySet())
                            {
                                ...
                                if(entry.getKey()==round)
                                {
                                    ...
                                    if(round != (int)(Math.floor(round/x)*x))
                                    {
                                        ...
                                        co_ag_pair.count += entry.getValue().count;
                                        co_ag_pair.aggregation = co_ag_pair.aggregation + entry.getValue().aggregation;
                                        co_ag_pair.aggregation_square = co_ag_pair.aggregation_square + entry.getValue().aggregation_square;
                                ...
                                else//Line 11-21 in Algorithm 1
                                {
                                    ...
                                    if(entry.getKey() != (int)(Math.floor(entry.getKey()/x)*x) && (int)(Math.floor(entry.getKey()/x)) != k+1)
                                    {
                                        ...
                                        co_ag_pair1.count += entry.getValue().count;
                                        co_ag_pair1.aggregation = co_ag_pair1.aggregation + entry.getValue().aggregation;
                                        co_ag_pair1.aggregation_square = co_ag_pair1.aggregation_square + entry.getValue().aggregation_square;
                            ...
                            System.out.println("At time: "+ current_time + ", Aggregating results:" + Math.sqrt((Result.aggregation_square-2*Result.aggregation*(Result.aggregation/Result.count)+Result.count*Math.pow((Result.aggregation/Result.count), 2))/Result.count));
                            ...
                            if(round%x == x-1 || round == n-1)
                            {
                                ...
                                {
                                    count_aggregation co_ag_pair = cmix[next_k*x];
                                    co_ag_pair.count += cmix[j*x].count;
                                    co_ag_pair.aggregation = co_ag_pair.aggregation + cmix[j*x].aggregation;
                                    co_ag_pair.aggregation_square = co_ag_pair.aggregation_square + cmix[j*x].aggregation_square;
                                    cmix[next_k*x] = co_ag_pair;
                                }
                            }
    ...
    public static count_aggregation Compute_Result(count_aggregation[] cmix, count_aggregation Result, int k, int next_k, int m, int n, int x)
    {
        ...
        if(k < Math.ceil(n/x)-1)
        {
            ...
            {
                Result.aggregation_square = cmix[0].aggregation_square + cmix[next_k*x].aggregation_square;
                Result.aggregation = cmix[0].aggregation + cmix[next_k*x].aggregation;
                Result.count = cmix[0].count + cmix[next_k*x].count;
            }
            else//Otherwise (Lines 5-6 in Algorithm 2)
            {
                Result.aggregation_square = cmix[0].aggregation_square + cmix[next_m].aggregation_square + cmix[next_k*x].aggregation_square;
                Result.aggregation = cmix[0].aggregation + cmix[next_m].aggregation + cmix[next_k*x].aggregation;
                Result.count = cmix[0].count + cmix[next_m].count + cmix[next_k*x].count;
            }
        }
        else//If the current block is the ⌈n/x⌉ − 1 th block (Line 8 in Algorithm 2):
        {
            ...
            {
                Result.aggregation_square = cmix[0].aggregation_square + cmix[next_m].aggregation_square;
                Result.aggregation = cmix[0].aggregation + cmix[next_m].aggregation;
                Result.count = cmix[0].count + cmix[next_m].count;
            }
            else// Otherwise (Lines 11-12 in Algorithm 2)
            {
                Result.aggregation_square = cmix[0].aggregation_square;
                Result.aggregation = cmix[0].aggregation;
                Result.count = cmix[0].count;
            }
        }
        return Result;
    }
    ...
    public static count_aggregation[] Aggkplus1Block(count_aggregation[] cmix, int k, int next_k, int m, int n, int x)
    {
        ...
        for(int i = block-2; i>=0; i--)
        {
            ...
            co_ag_pair.count += cmix[next_k*x+i+1].count;
            co_ag_pair.aggregation = co_ag_pair.aggregation + cmix[next_k*x+i+1].aggregation;
            co_ag_pair.aggregation_square = co_ag_pair.aggregation_square + cmix[next_k*x+i+1].aggregation_square;
    ...
    public static count_aggregation[] UpdatePCResult(count_aggregation[] pwix, Map<Integer, count_aggregation> pvalue_perslide, int p, int m, count_aggregation R, long start_time, int slide)
    {
        ...
        double la_pre_square = 0;
        double la_pre = 0;
        int la_pre_count = 0;
        Pair_value collective_result;
        if(!pvalue_perslide_pwix_r.isEmpty())
        {
            collective_result = UPResult(pwix, pvalue_perslide_pwix_r, sortedKeys_r, la_pre_square, la_pre, la_pre_count, p-1, have_right, start_time, slide);
            pwix = collective_result.pwix;
            la_pre_square = collective_result.la_pre_square;
        ...
        {
            collective_result = UPResult(pwix, pvalue_perslide_pwix_l, sortedKeys_l, la_pre_square, la_pre, la_pre_count, m%p, have_right, start_time, slide);
            pwix = collective_result.pwix;
            //la_pre = collective_result.la_pre;
            //la_pre_count = collective_result.la_pre_count;
        }
    ...
    public static Pair_value UPResult(count_aggregation[] pwix, Map<Integer, count_aggregation> pvalue_perslide_in_pwix, ArrayList<Integer> sortedKeys, double la_pre_square, double la_pre, int la_pre_count, int lst, boolean have_right, long start_time, int slide)
    {
        ...
        for(int i=l; i<lst; i++)
        {
            ...
            {
                la_pre_square = la_pre_square + pvalue_perslide_in_pwix.get(i).aggregation_square;
                la_pre = la_pre + pvalue_perslide_in_pwix.get(i).aggregation;
                la_pre_count = la_pre_count + pvalue_perslide_in_pwix.get(i).count;
            }
            pwix[i].aggregation_square = pwix[i].aggregation_square + la_pre_square;
            pwix[i].aggregation = pwix[i].aggregation + la_pre;
            pwix[i].count = pwix[i].count + la_pre_count;
            System.out.println("Past Result of the affected window computed at: "+start_time+slide*1000*i + ", Updated result: " + Math.sqrt((pwix[i].aggregation_square-2*pwix[i].aggregation*(pwix[i].aggregation/pwix[i].count)+pwix[i].count*Math.pow((pwix[i].aggregation/pwix[i].count), 2))/pwix[i].count) );
        }
        collective_result.pwix = pwix;
        collective_result.la_pre_square = la_pre_square;
        collective_result.la_pre = la_pre;
        collective_result.la_pre_count = la_pre_count;
        return collective_result;
    }
    
    public static class Pair_value {
        count_aggregation[] pwix;
        double la_pre_square;
        double la_pre;
        int la_pre_count;
     }
