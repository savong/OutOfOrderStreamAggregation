 ...
 */
public class CMiX_PWiX_AVERAGE_using_Flink {
                    ...
                    if(current_time-Long.parseLong(v2.f0)<=max_lateness)
                    {
                        ...
                        if(duration < (round+1)*f1)
                        {
                            ...
                            else
                            {
                                ...
                                co_ag_pair.count = 1;
                                co_ag_pair.aggregation = (double)v2.f1;
                                pvalue_perslide.put(index_falling, co_ag_pair);
                            }
                        ...
                        else
                        {
                            ...
                            System.out.println("At time: "+ current_time + ", Aggregating results:" + Result.aggregation/Result.count);
    ...
    public static count_aggregation[] UpdatePCResult(count_aggregation[] pwix, Map<Integer, count_aggregation> pvalue_perslide, int p, int m, count_aggregation R, long start_time, int slide)
    {
        ...
        {
            collective_result = UPResult(pwix, pvalue_perslide_pwix_r, sortedKeys_r, la_pre, la_pre_count, p-1, have_right, start_time, slide);
        ...
        {
            collective_result = UPResult(pwix, pvalue_perslide_pwix_l, sortedKeys_l, la_pre, la_pre_count, m%p, have_right, start_time, slide);
            pwix = collective_result.pwix;
            //la_pre = collective_result.la_pre;
            //la_pre_count = collective_result.la_pre_count;
        }
    ...
    public static Pair_value UPResult(count_aggregation[] pwix, Map<Integer, count_aggregation> pvalue_perslide_in_pwix, ArrayList<Integer> sortedKeys, double la_pre, int la_pre_count, int lst, boolean have_right, long start_time, int slide)
    {
        ...
        for(int i=l; i<lst; i++)
        {
            ...
            System.out.println("Past Result of the affected window computed at: "+start_time+slide*1000*i + ", Updated result: " + pwix[i].aggregation/pwix[i].count );
        }
