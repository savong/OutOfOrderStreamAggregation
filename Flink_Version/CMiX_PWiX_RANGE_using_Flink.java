...
*/
public class CMiX_PWiX_RANGE_using_Flink {
    public static void main(String[] args) throws Exception {
        ...
        if(f2==0)//If window size is dividiable by slide size
        {
            ...
            .reduce(new ReduceFunction<Tuple2<String,Double>>(){
                ...
                {
                    //Becase we compute minimum value and maximum value, so the default values in both cmix and pwix must be set to DOUBLE.MAX_VALUE and DOUBLE.MIN_VALUE respectively.
                    if(initialize)
                    {
                        for(int i=0;i<n;i++)
                        {
                    ...
                    if(current_time-Long.parseLong(v2.f0)<=max_lateness)
                    {
                        ...
                        if(duration < (round+1)*f1)
                        {
                            if(pvalue_perslide.containsKey(index_falling))
                            {
                                ...
                                co_ag_pair.max = Math.max(co_ag_pair.max, v2.f1);
                                co_ag_pair.min = Math.min(co_ag_pair.min, v2.f1);
                                pvalue_perslide.put(index_falling, co_ag_pair);
                            }
                            else
                            {
                                count_aggregation co_ag_pair = new count_aggregation();
                                co_ag_pair.max = (double)v2.f1;
                                co_ag_pair.min = (double)v2.f1;
                                pvalue_perslide.put(index_falling, co_ag_pair);
                            }
